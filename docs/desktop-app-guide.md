...
技能页默认显示 **已安装**，后面依次是 **OpenAkita 官方市场** 和 **SkillHub**。两个市场分别浏览，搜索和筛选只作用于当前市场。

- **官方市场**：搜索技能、按分类筛选，并按热门、最新发布或获取量排序，支持分页浏览。
- **获取官方市场技能**：点击“获取”，在市场页面完成登录和获取，再通过现有流程回到 OpenAkita 安装。安装完成后，已安装列表会刷新。
- **安装后的名称**：市场技能优先显示安装时保存的市场正式名称，支持离线显示和按名称搜索。内部标识与 `SKILL.md` 保持不变；旧安装可从匹配资源 ID 和版本的成功安装历史恢复名称，缺少记录时沿用技能本地名称。
- **SkillHub**：保留关键词搜索和从 SkillHub 注册表下载安装的流程。
- **聊天中使用**：可以用市场正式名称指定技能，例如“请使用岗位定制简历技能”。助手的技能目录同时显示正式名称和内部 ID；重名时会列出候选 ID，请明确选择后再执行。
- **市场不可用**：显示重试和主动切换 SkillHub 的入口，不会自动混入其他来源的资源。
- **直接链接**：`#/skills` 打开已安装；`#/skills?source=official` 和 `#/skills?source=skillhub` 分别打开两个市场。
